...
export function exportAsCSV(list: List, getLabelFn: (id: string) => string): string {
  ...
  csv += '\n# Metadata\n'
  csv += `# List Name,"${list.name}"\n`
  csv += `# Exported At,"${metadata.exportedAt}"\n`
  csv += `# Total Items,${metadata.totalItems}\n`
  csv += `# Total Comparisons,${metadata.totalComparisons}\n`
  csv += `# Average Confidence,${metadata.averageConfidence}\n`
...
export function exportAsMarkdown(list: List, getLabelFn: (id: string) => string): string {
  ...
  md += '---\n'
  md += '*Generated by [Duel Ranker](https://duel-ranker.com)*\n'
